    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
      ...
      {showForm && (
        <form onSubmit={addReminder} className="bg-white border border-sage-200 rounded-xl p-5 mb-6 space-y-3">
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">Link to event (optional)</label>
            <select
              value={form.event_id}
              onChange={(e) => setForm({ ...form, event_id: e.target.value })}
              className={inputClass}
            >
              <option value="">— No event (standalone reminder) —</option>
              {events.map((ev) => (
                <option key={ev.id} value={ev.id}>
                  {ev.name} · {new Date(ev.event_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' })}
                  {ev.status === 'past' ? ' (past)' : ''}
                </option>
              ))}
            </select>
