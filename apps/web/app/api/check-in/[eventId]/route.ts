...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const eventRows = await sql`
      SELECT id, org_id FROM events WHERE (slug = ${eventId} OR id::text = ${eventId}) AND status != 'archived'
    `
    if (!eventRows[0]) throw new ApiError(404, 'Event not found')
    const orgId = eventRows[0].org_id
    // The URL param may be a slug — use the resolved uuid from here on
    const resolvedEventId = eventRows[0].id as string
    ...
    const existing = await sql`
      ...
      WHERE event_id = ${resolvedEventId} AND LOWER(email) = LOWER(${data.email})
      LIMIT 1
    `
    ...
    if (existing[0]) {
      ...
      const inserted = await sql`
        INSERT INTO rsvps (
          ...
          ${orgId}, ${resolvedEventId}, ${data.name}, ${data.email.toLowerCase()},
          ${responsesJson}::jsonb,
